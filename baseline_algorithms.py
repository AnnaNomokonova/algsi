...
# ─── Baseline PSO (vmax постоянный, без уменьшения) ────────────────────────────
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Нахождение локального минимума (baseline)")
...
class ParamWindow(tk.Toplevel):
    ...
    def __init__(self, parent, method, func="1d"):
        ...
        self.method = method
        self.func = func
        self.title("Генетический алгоритм (baseline)" if method == "ga"
                   else "Роевой алгоритм (PSO) (baseline)")
        self.resizable(False, False)
        self.entries = {}
        self._build()
    ...
    def _run(self):
        ...
        try:
            if self.func == "2d":
                if self.method == "ga":
                    bx, by, bval, history, snapshots = genetic_algorithm_2d_baseline(
                    ...
                    bx, by, bval, history, snapshots = pso_2d_baseline(
                ...
                if self.method == "ga":
                    best_x, best_y, history, snapshots = genetic_algorithm_baseline(
                        pop_size=self._get("pop_size", int),
                        generations=self._get("generations", int),
                        crossover_prob=self._get("crossover_prob"),
                        mutation_prob=self._get("mutation_prob"),
                    ...
                    best_x, best_y, history, snapshots = pso_baseline(
                        swarm_size=self._get("swarm_size", int),
                        iterations=self._get("iterations", int),
                        c1=self._get("c1"),
                        c2=self._get("c2"),
